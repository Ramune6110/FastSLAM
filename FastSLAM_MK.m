    ...
    for ip = 1 : NP
        ...
        for iz=1:length(z(:, 1))
            if (flag(iz, ip) < 1.0)
                % LMを初めて観測する場合
                % 観測値の平均値の初期化
                mu(3 * ip - 2 : 3 * ip - 1, iz) = xEst(1 : 2) + [z(iz, 1) * cos(xEst(3) + z(iz, 2)); z(iz, 1) * sin(xEst(3) + z(iz, 2))]; % (8.44)式
                % ヤコビアンH行列
                H = jacobian_H(z, x);
                % 観測値の共分散行列初期化
...
function H = jacobian_H(z, x)
    % ヤコビアン行列H
    H = [(z(3) - x(1)) / z(1) (z(4) - x(2)) / z(1)^2;
         (x(2) - z(4)) / z(1) (z(3) - x(1)) / z(1)^2];
end
...
function [expectedZ, H] = measurement_model(mu, x)
    ...
    H         = [(mu(1) - x(1)) / expectedZ(1) (mu(2) - x(2)) / expectedZ(1)^2;
                 (x(2) - mu(2)) / expectedZ(1) (mu(1) - x(1)) / expectedZ(1)^2];
end
